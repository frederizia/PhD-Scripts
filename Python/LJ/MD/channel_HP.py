...
def main():
    ...
    mdot_avg = np.mean(np.array(mdot))
    print 'The average mass flow rate is', mdot_avg
    ...
    # rho centre NEMD only

    m,b = np.polyfit(xvals, rhovals[MIDP,:], 1)
    ...
    #plt.legend()
    #plt.ylim(0.1,0.7)
    plt.savefig('PLOTS/rhovals_centre_nemd_%s.png'%name_nemd)
    #plt.show()
    ...
    plt.savefig('PLOTS/massflow_%s.png'%name_nemd)
